...
function calcula()
{
	...
	$("#texto").append("<style> .resultado{margin: 10px auto;background: #00b48e;width: 97%;text-align: center;} .resultado th{	background: #00b48e; padding: 5px; color:#fff;} .resultado td{ background: #fff; padding: 5px;} .resultado tfoot td{background: #00b48e; color:#fff; }</style>");
	...
	for(var or in origenes)for(var de in destinos)//if(origen > 0 && destino > 0)
	{
		...
		//por cada combinación de origen y destino lo muestro en pantalla para que puedan elegir
		lterminales = "";
	html="";
	...
	console.log("####");
	console.log("Ruta elegida: " + lterminales + " TTE:"+tte+" - "+precio +"€");
	console.log("####");
	...
	for(var i in tramos){
		if(salto[tramos[i]]['bf']==1){
			if(terminales[salto[tramos[i]]['origen']]['latlong'] != '' && terminales[salto[tramos[i]]['destino']]['latlong'] != '')
			{
				...
				console.log("Distancia tramo internacional:"+tramo_distancia);
